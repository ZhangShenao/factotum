...
whenToUse: 用户要求代码审查、review 变更、提交前检查（原 OpenCode /cr 命令）时使用。仅静态只读审查，不改仓库状态。
---

# Review Working Tree

（自 OpenCode `.opencode/commands/cr.md` 迁移，逻辑不变；`/cr` 引用改为本 skill。）
...
| "Run tests/build, fix, and commit" | Do none within this skill; report static findings and the verification gap |

...
whenToUse: 用户要求推送分支、开 PR、创建 draft pull request（原 OpenCode /pr 命令）时使用。
---

# Open a Draft Pull Request

（自 OpenCode `.opencode/commands/pr.md` 迁移，逻辑不变；`/commit` 引用改为 `commit` skill。）
...
- Never commit inside this workflow; require a prior `commit` skill run or explicit user authorization of existing HEAD
...
If there is no `commit` skill result in the current conversation, ask the user to explicitly authorize publishing the existing HEAD before proceeding.
...
- Set `ORIGIN_REPO` = `<owner>/<repo>` from that push URL **before** calling `gh repo view`, then `BASE_REPO` = `ORIGIN_REPO` (if not a fork), `BASE_BRANCH` = default branch from `gh repo view`.
...
| No commit evidence | Ask user to authorize existing HEAD or run the `commit` skill first |
